...
get_my_coord <- function(x){
  ...
  raster_2020 <- raster_muni_y == x
  raster_2020[raster_2020 == 1] <- x # ENTENDER O QUE OCORRE
  # names(raster_2020) <- "Desmatamento_2020"
  idx <- which(raster_muni_y[] == x)  # índice dos pixels com valor 22
  coords <- xyFromCell(raster_muni_y, idx)  # coordenadas dos pixels
  as.data.frame(coords) |>   # data.frame com x (lon) e y (lat)
    add_column(categorie = x,
               muni = muni_y$name_muni,
               state = muni_y$abbrev_state)
    }
df_final <- map_df(codigo[1],get_my_coord)
```


```{r, eval=FALSE}
df_final |> 
  ggplot(aes(x,y,color = as_factor(categorie))) +
  geom_point()
```
