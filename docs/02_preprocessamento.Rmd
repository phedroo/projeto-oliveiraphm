...
# Filtrado somente para o setor de agricultura
# write_rds(emissions_sources, "../data/emissions_sources_final.rds")
```
...
# Salvar base sem outliers
appeears_modis <- municipality |>
  filter(abbrev_state %in% my_states) |> 
  left_join(
    appeears_modis |> 
      group_by(year) |>  
    filter(FPAR <= 250, LAI <= 250),
    by = "name_muni"
  ) |>
  filter(year == my_year)
