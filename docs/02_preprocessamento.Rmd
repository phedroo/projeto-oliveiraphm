...
A base CT nova contém dados para o período de 2021 a 2025, enquanto a base antiga, para o perídoo de 2015 a 2023. Os dados desta (antiga) foram utilizados até o ano de 2020.
...
# Atualizando a base ct para corresponder à nova
emissions_sources_15_20 <- emissions_sources |>
  filter(year %in% 2015:2020, # só até 2020, pois a nova contém a partir de 2021
         sigla_uf %in% my_states,
         gas == "co2e_100yr") |> 
  select(year, city_ref, sigla_uf, emissions_quantity, sector_name, sub_sector) |>
  ...
  mutate(
    city_ref = stri_trans_general(tolower(city_ref), "Latin-ASCII"),
    city_ref = trimws(city_ref),
    setor = stri_trans_general(tolower(setor), "Latin-ASCII"),
    setor = trimws(setor),
    subsetor = stri_trans_general(tolower(subsetor), "Latin-ASCII"),
    subsetor = trimws(subsetor)
  ) |> 
  group_by(year, state, city_ref, subsetor) |>
  summarise(emissao_sub_setores = sum(emissao)) |> 
  pivot_wider(
...
# Padronizando nomes dos setores e subsetores para a nova nomenclatura
# Vetor antigo e novo
setores_antigos <- c(
  "agriculture", "forestry_and_land_use", "fossil_fuel_operations",
  "manufacturing", "transportation", "power",
  "waste", "mineral_extraction"
)

setores_novos <- c(
  "Agricultura", "Florestas e Uso da Terra", "Manufatura", "Transporte", "Energia", "Resíduos",  "Operações de Combustíveis Fósseis", "Extração Mineral"
)

# Vetor de nomes para usar no rename
map_setores <- setNames(setores_novos, setores_antigos)

subsetores_antigos <- c(
  "cropland-fires", "enteric-fermentation-cattle-feedlot",
  "enteric-fermentation-cattle-pasture", "manure-left-on-pasture-cattle",
  "manure-management-cattle-feedlot", "rice-cultivation",
  "synthetic-fertilizer-application", "forest-land-clearing",
  "forest-land-degradation", "forest-land-fires", "net-forest-land",
  "net-shrubgrass", "net-wetland", "removals", "shrubgrass-fires",
  "water-reservoirs", "wetland-fires", "coal-mining",
  "oil-and-gas-production-and-transport", "oil-and-gas-refining",
  "aluminum", "cement", "chemicals", "pulp-and-paper", "steel",
  "bauxite-mining", "copper-mining", "iron-mining",
  "electricity-generation", "domestic-aviation", "domestic-shipping",
  "international-aviation", "international-shipping",
  "road-transportation", "solid-waste-disposal",
  "wastewater-treatment-and-discharge"
)

subsetores_novos <- c(
  "Queimadas em áreas agrícolas", "Fermentação entérica – gado confinado",
  "Fermentação entérica – gado a pasto", "Esterco deixado no pasto (gado)",
  "Manejo de esterco – gado confinado", "Cultivo de arroz",
  "Aplicação de fertilizantes sintéticos", "Forest land clearing",
  "Forest land degradation", "Forest land fires", "Uso líquido de terras florestais",
  "Uso líquido de áreas arbustivas e gramíneas", "Uso líquido de áreas úmidas",
  "Remoções de carbono (sequestro)", "Queimadas em áreas arbustivas",
  "Reservatórios de água", "Wetland fires", "Mineração de carvão",
  "Produção de petróleo e gás", "Refino de petróleo e gás",
  "Produção de alumínio", "Produção de cimento", "Indústria química",
  "Produção de papel e celulose", "Produção de ferro e aço",
  "Mineração de bauxita", "Mineração de cobre", "Mineração de ferro",
  "Geração de eletricidade", "Aviação doméstica", "Navegação doméstica",
  "Aviação internacional", "Navegação internacional",
  "Transporte rodoviário", "Disposição de resíduos sólidos",
  "Tratamento e descarte de esgoto doméstico"
)
map_subsetores <- setNames(subsetores_novos, subsetores_antigos)

emissions_sources_15_20_renomeada <- emissions_sources_15_20 |> 
  rename_with(~ str_replace_all(., map_setores), .cols = starts_with("emissao_")) |> 
  rename_with(~ str_replace_all(., map_subsetores), .cols = starts_with("emissao_")) 

# Salvar
# write_rds(emissions_sources_15_20_renomeada, "../data/emissions_sources_15_20.rds")
```

```{r,eval=FALSE}
# Carregando base antiga
base_antiga <- read_rds("../data/emissions_sources_15_20.rds") 

# Adicionando base e padronizando colunas que começam com emissao
bases_ct_atualizadas <- nova_base_ct |> full_join(
  base_antiga,
  # selecionar somente as colunas com nomes idênticos nas duas bases:
  by = intersect(names(nova_base_ct), names(base_antiga)))|> 
  # ----------------------------------------
  select(-ends_with(".x"), -ends_with(".y"))

bases_ct_atualizadas # ta considerando as regiões administrativas da nova base

# write_rds(bases_ct_atualizadas, '../data-raw/bases_ct_atualizadas.rds')
```
