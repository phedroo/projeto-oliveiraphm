...
df <- base_completa |>
  group_by(year,state) |>
  summarise(
    N = length(.data[[variavel]]), # n° de observações
    MIN = min(.data[[variavel]]), # valor mínimo
    MEAN = mean(.data[[variavel]]), # média
    MEDIAN = median(.data[[variavel]]), # mediana
    MAX = max(.data[[variavel]]), # valor máximo
    VARIANCIA  = var(.data[[variavel]]), 
    STD_DV = sd(.data[[variavel]]), # desvio padrão
    CV = 100*STD_DV/MEAN, # coeficiete de variação
    SKW = agricolae::skewness(.data[[variavel]]), #
    KRT = agricolae::kurtosis(.data[[variavel]]), #
  )
# Salvar
  # rodar sempre que alterar a variavel:
